...
const EdubreakService = {
  ...
  getInbox: async function () {
    ...
    try {
      const data = await fetch(this.getEdubreakApiUrl() + '/svb/' + boardId + '/inbox', options)
        .then(response => {
          return response.json()
        })
      console.log('### EdubreakService: inbox is ###', data.data)
      return data.data;
    } catch (e) {
      console.log('### EdubreakService: there is no new inbox items for this board ###', e);
    }
  },
  ...
  deleteFromInbox: async function (nid: string) {
    ...
    try {
      const data = await fetch(this.getEdubreakApiUrl() + '/svb/' + boardId + '/inbox/' + nid, options)
        .then(response => {
          return response.json()
        })
