...
const EdubreakService = {
	...
	getNodeAsJSON: async function (options: any) {
		const endpoint = await this.getEdubreakEndpointFromType(options)
		const headers = { Authorization: 'Bearer ' + this.getEdubreakAccessToken() }
		return await fetch(this.getEdubreakApiUrl() + endpoint + options.nid, { headers }).then(
			(response) => {
	...
	parseEdubreakLink: function (text: string) {
		...
		const edubreakIds = EdubreakService.getEdubreakIds(path)
		return {
			domain: domain,
			path: path,
	...
	saveStateToEdubreak: async function (state: any) {
		...
		try {
			...
			console.error(
				'### EdubreakService: error while persisting document state to the server ###',
				e
			)
		}
	},
	...
	getStateFromEdubreak: async function () {
		...
		try {
			const data = await fetch(this.getEdubreakApiUrl() + '/svb/' + boardId, options).then(
				(response) => {
					return response.json()
				}
			)
	...
	getInbox: async function () {
		...
		try {
			const data = await fetch(
				this.getEdubreakApiUrl() + '/svb/' + boardId + '/inbox',
				options
			).then((response) => {
				return response.json()
			})
	...
	deleteFromInbox: async function (nid: string) {
		...
		try {
			const data = await fetch(
				this.getEdubreakApiUrl() + '/svb/' + boardId + '/inbox/' + nid,
				options
			).then((response) => {
				return response.json()
			})
