...
/** @public */
export type TLPresentShapePermissions = { [K in TLPresentShapePermissionName]?: boolean }
...
// --- VALIDATION ---
/** @public */
export const presentShapeTypeValidator: T.Validator<TLPresentShape> = createShapeValidator(
	'present',
	T.object({
		...
		overridePermissions: T.dict(
			T.setEnum(
				new Set(Object.keys(tlPresentShapePermissionDefaults) as TLPresentShapePermissionName[])
			),
			T.boolean.optional()
		).optional(),
	})
...
/** @public */
export const PRESENT_DEFINITIONS = [
	{
		type: 'tldraw',
		title: 'tldraw',
...
] as const satisfies readonly PresentDefinition[]
...
/** @public */
export const presentShapeMigrations = defineMigrations({
	...
	migrators: {
		...
		[Versions.GenOriginalUrlInEmbed]: {
			...
			up: (shape) => {
				...
				const host = new URL(url).host.replace('www.', '')
				let originalUrl
				for (const localEmbedDef of PRESENT_DEFINITIONS) {
					if ((localEmbedDef as PresentDefinition).hostnames.includes(host)) {
						try {
